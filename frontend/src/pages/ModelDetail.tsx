      ...
      <main className="pt-16">
        <div className="container mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="flex items-center gap-3 mb-6 sm:mb-8">
            <button onClick={() => navigate('/')} className="flex items-center gap-2 px-3 py-2 text-gray-500 hover:text-cyan-600 bg-white border border-gray-200 rounded-xl hover:border-cyan-200 hover:bg-cyan-50/50 shadow-sm transition-all duration-200 active:scale-[0.97]">
              <ArrowLeft size={16} />
              <span className="text-sm font-medium">返回</span>
            </button>
            <div className="flex-1 min-w-0">
              <h1 className="text-xl sm:text-2xl font-bold text-gray-800 truncate">{displayName}</h1>
          ...
          <div className="flex gap-6 flex-col lg:flex-row">
            ...
            <div className="flex-shrink-0 flex items-start justify-center">
              <div ref={mockPageRef} className="w-full sm:w-[800px] h-[320px] sm:h-[500px] bg-[#1a1a2e] border border-gray-200 rounded-2xl overflow-hidden relative flex flex-col shadow-xl">
                {/* 模拟浏览器头部 */}
                <div className="h-8 bg-[#16213e] border-b border-white/5 flex items-center px-3 gap-[6px] flex-shrink-0">
                  ...
                  <div className="flex-1 mx-4">
                    <div className="h-4 bg-white/[0.06] rounded-md max-w-[200px] mx-auto" />
                  </div>
                </div>
                {/* 模拟网页内容 */}
                <div className="flex-1 p-4 sm:p-6 flex flex-col gap-2.5">
                  <div className="h-2.5 bg-white/[0.06] rounded-md w-4/5" />
                  <div className="h-2.5 bg-white/[0.06] rounded-md w-3/5" />
                  <div className="h-2 bg-white/[0.04] rounded-md" />
                  <div className="h-2 bg-white/[0.04] rounded-md w-4/5" />
                  <div className="h-2 bg-white/[0.04] rounded-md" />
                  <div className="h-2 bg-white/[0.04] rounded-md w-3/5" />
                  <div className="h-2 bg-white/[0.04] rounded-md" />
                </div>
                {/* 模拟 Live2D 模型位置 - 可拖拽 */}
                <div
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-4">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">位置</label>
                  <div className="flex bg-gray-50 rounded-xl p-1 gap-1">
                    <button
                      onClick={() => updateGen({ position: 'left' })}
                      className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${genState.position === 'left' ? 'bg-white text-cyan-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >左下</button>
                    <button
                      onClick={() => updateGen({ position: 'right' })}
                      className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${genState.position === 'right' ? 'bg-white text-cyan-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >右下</button>
                  </div>
                </div>
                ...
                <div className="flex gap-3">
                  <div className="flex-1 flex flex-col gap-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">水平偏移</label>
                    <input type="number" min={0} max={500} value={genState.offsetX}
                      onChange={e => updateGen({ offsetX: parseInt(e.target.value) || 0 })}
                      className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-sm font-mono outline-none input-focus"
                    />
                  </div>
                  <div className="flex-1 flex flex-col gap-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">垂直偏移</label>
                ...
                <div className="flex gap-3">
                  <div className="flex-1 flex flex-col gap-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">宽度</label>
                  ...
                  <div className="flex-1 flex flex-col gap-2">
                    <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">高度</label>
                ...
                {/* 缩放 */}
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">缩放比例</label>
                  <div className="flex bg-gray-50 rounded-xl p-1 gap-1">
                    {[0.5, 1, 1.5, 2].map(s => (
                      <button key={s} onClick={() => updateGen({ scale: s })}
                        className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${genState.scale === s ? 'bg-white text-cyan-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                      >{s}x</button>
                    ))}
                  </div>
                </div>
                ...
                {/* 移动端隐藏 */}
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">移动端隐藏</label>
                  <div className="flex bg-gray-50 rounded-xl p-1 gap-1">
                    <button onClick={() => updateGen({ hideOnMobile: true })}
                      className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${genState.hideOnMobile ? 'bg-white text-cyan-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >隐藏</button>
                ...
                <div className="flex flex-col gap-2">
                  <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
                    聊天内容 <span className="text-gray-300 text-[11px] normal-case tracking-normal font-normal">每行一条消息</span>
                  </label>
                  <textarea rows={4} value={messagesText}
                    onChange={e => handleMessagesChange(e.target.value)}
                    placeholder="你好呀~&#10;今天天气真好!&#10;有什么想问的吗?&#10;欢迎来到这里~"
                    className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-mono outline-none resize-y min-h-[80px] leading-relaxed input-focus"
                  />
                </div>
              </div>
              ...
              <div className="code-block">
                <div className="flex items-center justify-between bg-gradient-to-r from-gray-50 to-gray-100/50 px-4 py-2.5 border-b border-gray-200/50">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-gray-400 ml-2">{genState.isCubism4 ? 'Cubism 4' : 'Cubism 2'} &middot; HTML + JavaScript</span>
                  </div>
                  <button onClick={handleCopy}
                    className="btn-hover flex items-center gap-1.5 px-4 py-1.5 text-xs font-medium text-white bg-gradient-to-r from-cyan-500 to-blue-500 rounded-lg shadow-md shadow-cyan-500/20 hover:shadow-lg hover:shadow-cyan-500/30 transition-all duration-200"
                  >
